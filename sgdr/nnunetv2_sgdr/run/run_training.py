...
from nnunetv2_sgdr.paths import nnUNet_preprocessed
from nnunetv2_sgdr.run.load_pretrained_weights import load_pretrained_weights
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
                          ...
                          num_epochs:int = 1000, initial_lr:float = 1e-1, t_0:int = 10, t_mult:int = 2, eta_min = 1e-4): #$ added num_of_cycles, epochs and gamma
    # load nnunet class and do sanity checks
    nnunet_trainer = recursive_find_python_class(join(nnunetv2_sgdr.__path__[0], "training", "nnUNetTrainer"),
                                                trainer_name, 'nnunetv2.training.nnUNetTrainer')
    if nnunet_trainer is None:
        raise RuntimeError(f'Could not find requested nnunet trainer {trainer_name} in '
                           f'nnunetv2.training.nnUNetTrainer ('
                           f'{join(nnunetv2_sgdr.__path__[0], "training", "nnUNetTrainer")}). If it is located somewhere '
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    ...
                                    num_epochs= num_epochs , initial_lr=initial_lr, t_0= t_0, t_mult=t_mult, eta_min=eta_min      )  #$ added num_of_cycles, epochs and gamma
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 t_mult:int = 2, 
                 eta_min = 1e-4
                 ): #$ added num_epochs, num_of_cycles and gamma
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               ...
                                               num_epochs= num_epochs , initial_lr=initial_lr, t_0= t_0, t_mult=t_mult, eta_min=eta_min) #$ added num_epochs, num_of_cycles and gamma
...
def run_training_entry():
    ...
    parser.add_argument('-initial_lr', type=float, default=1e-1, required=False,
                    help="Initial learning rate")
    parser.add_argument('-t_0', type=int, default=10, required=False,
                    help="Number of epochs before the first restart")
    parser.add_argument('-t_mult', type=int, default=2, required=False,
                    help="Duration of each cycle (number of epochs in each cycle is multiplied by this argument)")
    parser.add_argument('-eta_min', type=float, default=1e-3, required=False,
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, num_epochs=args.num_epochs, initial_lr=args.initial_lr, t_0=args.t_0, t_mult=args.t_mult, eta_min=args.eta_min)

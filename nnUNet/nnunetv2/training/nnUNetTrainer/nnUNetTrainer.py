...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.early_stop_patience = 5  # Number of epochs to wait before stopping training
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            # We use the last value of the validation metric for early stopping
            current_val_metric = self.logger.my_fantastic_logging['ema_fg_dice'][-1]
            ...
            self.print_to_log_file(
                f"Epoch {epoch}: current_val_metric = {current_val_metric:.4f}, "
                f"best = {self.best_val_metric:.4f}, early_stop_counter = {self.early_stop_counter}/{self.early_stop_patience}"
            )

            if self.early_stop_counter >= self.early_stop_patience:
                self.print_to_log_file("Early stopping triggered. No improvement for {} epochs.".format(self.early_stop_patience))
                break
